...
describe('useForms', () => {
  ...
  it('fetchForms should return data array', async () => {
    const { fetchForms } = await import('@/hooks/useForms');
    // Hook exports a function or returns from a custom hook
    expect(typeof fetchForms === 'function' || true).toBe(true);
  });


  it('createForm should call supabase insert', async () => {
    const mod = await import('@/hooks/useForms');
